<?php
	...
	if ($idSocio == ""){
		...
		if ($accion == "p"){ //Datos del usuario
			...
			} else if ($datosMedicos['Lateralidad'] == 0){
		...
		} else if ($accion == "a"){ //Lista de eventos con inscriptos a aprobar
...
group by e.idEvento");
		...
		} else if ($accion == "i"){ //Lista de eventos disponibles para inscribirse
						...
						if ($eventosDisponibles['Habilitado'] == 0){
							...
							if ($eraup['idTipoEvento'] == $eventosDisponibles['idTipoEvento']){
								if (strpos($distritos, $distrito['Nombre'])) { 
									...
									} else if ($eventosDisponibles['FechaInicioInscripcion2'] < $hoy && $eventosDisponibles['FechaFinInscripcion2'] > $hoy && $eventosDisponibles['Habilitado'] == 1){
										$fechaC = split(" ", $eventosDisponibles['FechaFinInscripcion2']);
										$fechaCF = split("-", $fechaC[0]);
									
										$tpl->assign("cierre", "Fecha de cierre Fase II: " . $fechaCF[2] . "/" . $fechaCF[1] . "/" . $fechaCF[0]);		
									} else if ($eventosDisponibles['FechaFinInscripcion2'] < $hoy && $eventosDisponibles['Habilitado'] == 1){
										$tpl->assign("cierre", utf8_encode("Inscripción cerrada"));	
									}
								}
							} else {

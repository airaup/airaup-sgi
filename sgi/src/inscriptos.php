<?php
	...
	if ($logueado == ""){
		...
		if ($accion == "eliminarS"){ //Si se elimina la inscripción
			...
			for ($i=1;$i<=$cantBorrar;$i++){
				...
				//Si estaba aprobado previamente, se resta una asistencia al tipo de evento y se elimina el registro histórico del socio
				if ($aprobado['Aprobado'] == 1 || $aprobado['Aprobado'] == 4){
					...
					if ($inscriptos['TipoEvento'] == "E.R.A.U.P."){
						//Se elimina la asistencia histórica a la mesa
				...
				if ($aprobado['Aprobado'] != 2){
					...
					//Se envía correo al inscripto para notificarle la baja
					try {
						enviarCorreo($direccion['Email'], utf8_encode("Baja de inscripción"), utf8_encode("Tu inscripción a ") . $evento['Nombre'] . utf8_encode(" fue dada de baja."));
					} catch (Exception $e){
						$errores++;
					}
				}
			...
			if ($errores == 0){
				if ($accion2 == 1){
					$mensaje = utf8_encode("El cambio fue realizado con éxito, ambos socios fueron notificados por correo.");
				} else {
					...
					if ($controlar && $cuposAbiertos > 0){
						$mensaje .= $cuposAbiertos . utf8_encode(" inscriptos dejaron de estar en lista de espera y fueron notificados, así como también los responsables de su aprobación.");
					}
				}
				...
				if ($accion2 == 1){
					$mensaje = utf8_encode("El cambio fue realizado con éxito pero ocurrieron errores al enviar las notificaciones por correo.");
				} else {
			...
			if ($accion == "v"){
				...
				//No lo dejo pasar si no pertencece a algún distrito organizador y es admin del mismo y/o RDR
			...
			if ($evento['idTipoEvento'] == $eraup['idTipoEvento'] && $hoy < $evento['FechaInicioInscripcion2'] && $representante){
				...
				} else if ($esOrganizador == "SI" && $accion == "v"){
					$tpl->assign("inscriptos", $cantidadTotal['Cantidad'] . " registros");
				} else {
			...
			while ($inscriptos=$conexion->Siguiente()){
				...
				} else if ($inscriptos['Vegetariano'] == 1){
					if ($inscriptos['Dieta'] != ""){
				...
				if ($inscriptos['Eliminado'] == 0){
					...
					} else if ($inscriptos['Aprobado'] == 1){
						$tpl->assign("aprobado", "Aprobado");
					} else if ($inscriptos['Aprobado'] == 2){
						$tpl->assign("aprobado", "Reprobado");
					} else if ($inscriptos['Aprobado'] == 3){
						$tpl->assign("aprobado", "En lista de espera");
					} else if ($inscriptos['Aprobado'] == 4){
						$tpl->assign("aprobado", "Aprobado, en lista de espera");
					}
				} else if ($inscriptos['Eliminado'] == 1){
					if ($inscriptos['Aprobado'] == 0){
				...
				if ($accion != "v"){
					...
					if ($evento['Evento'] == "E.R.A.U.P." && $adminE['idSocio'] == $logueado){
						$entro = true;
						$eraup = true;
					} else if ($evento['Evento'] != "E.R.A.U.P." && ($representante || $adminE['idSocio'] == $logueado)){
						$entro = true;
					}
	...
	function descargarListado($conexion, $conexion2, $presidente, $representante, $accion, $datosSocio, $idEvento, $nombreE){
		...
		$columnas = "Nombres;Apellidos;Documento;Fecha de Nac.;Email;Distrito;Club;Sexo;Rueda;Calidad de asistencia;Transporte;Es vegetariano;Consideraciones en la dieta;Fecha de inscripción;Estado de aprobación;Monto pagado;Cotización;Fecha de pago;Moneda;Observaciones;Persona de contacto;Teléfono de contacto;Relación;Obra social;Número de socio;Grupo sanguíneo;Factor;Enfermedades crónicas;Internación ult. 3 años;Enfermedades infecciosas;Intervenciones quirúrjicas;Alergias;Fuma;Lateralidad;Lentes;Audífonos;Limitaciones físicas;Donante de órganos;Donante de médula;Medicamentos;Nombre de la droga;Cantidad suministrada";
		...
		while($socios=$conexion->Siguiente()){
			...
			if ($socios['Eliminado'] == 0){
				...
				} else if ($socios['Aprobado'] == 1){
					$linea .= "Aprobado";
				} else if ($socios['Aprobado'] == 2){
					$linea .= "Reprobado";
				} else if ($socios['Aprobado'] == 3){
					$linea .= "En lista de espera";
				} else if ($socios['Aprobado'] == 4){
					$linea .= "Aprobado, en lista de espera";
				}
			} else if ($socios['Eliminado'] == 1){
				if ($socios['Aprobado'] == 0){
		...
		for ($x=0;$x<sizeof($arrayIDs);$x++){
			...
			if ($reprobado['Reserva'] == 0){
				...
				if ($hoy < $infoEvento['FechaInicioInscripcion2']){ //FASE I
					if ($reprobado['Rueda'] == "Rotaract" && $reprobado['Distrito'] != "Otro"){ //Rotaractiano de AIRAUP
					...
					} else if ($reprobado['Rueda'] == "Rotary" && $reprobado['Distrito'] != "Otro"){ //Rotario de AIRAUP
						...
						} else if ($cupoRotario == 0 && $esOrganizador != "SI"){ //No hay límite de Rotarios
					...
					} else if ($reprobado['Distrito'] == "Otro"){ //Extranjero
						...
						} else if ($cupoExtranjeros == 0){ //No hay límite de extranjeros
							//Se chequea cupo global
							$cupoRotario = ($valores['PorcentajeRotarios1'] * $valores['CupoMaximo']) / 100;
							$cupoTotal = $valores['CupoMaximo'] - $valores['Reserva'] - $cupoRotario - $cupoExtranjeros;
							$organizadores = obtenerOrganizadores($conexion, $idEvento, "", true);
				...
				} else if ($hoy >= $infoEvento['FechaInicioInscripcion2']){ //FASE II
					...
					if ($cantidad > 0){
						...
						while ($ingreso=$conexion->Siguiente()){
							if ($ingreso['Rueda'] == "Rotaract" && $ingreso['Distrito'] != "Otro"){ //Es Rotaractiano de AIRAUP el que sube
								$entra = true;
								break;
							} else if ($ingreso['Rueda'] == "Rotary" && $ingreso['Distrito'] != "Otro"){ //Es Rotario de AIRAUP el que sube	
								...
								if ($cupoRotario > 0 && $cantidadR['Cantidad'] < $cupoRotario){ //Existe límite y queda lugar
									$entra = true;
									break;
								} else if ($cupoRotario == 0){ //No existe límite y queda lugar en el general
									$entra = true;
									break;
								}
							} else if ($ingreso['Distrito'] == "Otro"){ //Es extranjero el que sube
								...
								if ($cupoExtranjeros > 0 && $cantidadE['Cantidad'] < $cupoExtranjeros){ //Existe límite y queda lugar
									$entra = true;
									break;
								} else if ($cupoExtranjeros == 0){ //No existe límite y queda lugar en el general
									$entra = true;
									break;
								}
							}
	...
	function movimientoDeCupos($conexion, $conexion2, $idEvento, $cantBorrar, $idPeriodoActual){
		...
		while ($espera=$conexion->Siguiente()){
			...
			if ($aprobado == 0){
				$mensajeI = utf8_encode("Se liberaron cupos en ") . $espera['Evento'] . utf8_encode(" y saliste de la lista de espera. Tu inscripción sigue pendiente de aprobación.");
				$mensajeR = utf8_encode("Se liberaron cupos en " ). $espera['Evento'] . " y " . $espera['Nombres'] . " " . $espera['Apellidos'] . utf8_encode(" salió de la lista de espera. Aún debes aprobar su inscripción.");
			} else if ($aprobado == 1){
				$mensajeI = utf8_encode("Se liberaron cupos en ") . $espera['Evento'] . utf8_encode(" y saliste de la lista de espera. Tu inscripción fue aprobada previamente por lo que no tenes que realizar ninguna acción en el sistema.");
				$mensajeR = utf8_encode("Se liberaron cupos en ") . $espera['Evento'] . " y " . $espera['Nombres'] . " " . $espera['Apellidos'] . utf8_encode(" salió de la lista de espera. Su inscripción fue aprobada previamente por lo que no tenes que realizar ninguna acción en el sistema.");
			}

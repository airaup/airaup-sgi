<?php
	...
	if ($idSocio == ""){
		...
		if ($accion == "aprobacion"){ //Si confirmó la inscripción, se envían los correos correspondientes	
				...
				if ($aprobacion != "" || $aprobacion != "undefined"){
					...
					if ($aprobacion == 1){ //Si fue aprobado
						...
						if ($hoy <= $evento['FechaFin']){
							//Se envía correo de confirmación al inscripto, solo si el evento no finalizó, después no tiene sentido avisar
							if ($socio['Estado'] == "3"){
								enviarCorreo($socio['Email'], utf8_encode("Inscripción aprobada") ,utf8_encode("Tu inscripción a ") . $evento['Nombre'] . " (" . $organiza . ")" . utf8_encode(" fue aprobada, pero seguís en lista de espera."));
							} else {
								enviarCorreo($socio['Email'], utf8_encode("Inscripción aprobada") ,utf8_encode("Tu inscripción a ") . $evento['Nombre'] . " (" . $organiza . ")" . " fue aprobada.");
							}
						}
						...
						if ($hoy <= $evento['FechaFin']){
							//Se envía correo de confirmación al inscripto, solo si el evento no finalizó, después no tiene sentido avisar
							if ($evento['Tipo'] == 0){
								enviarCorreo($socio['Email'], utf8_encode("Inscripción reprobada"), utf8_encode("Tu inscripción a ") . $evento['Nombre'] . " (" . $organiza . ")" . " fue reprobada. Comunicate con tu Presidente.");
							} else if ($evento['Tipo'] == 1){
								enviarCorreo($socio['Email'], utf8_encode("Inscripción reprobada"), utf8_encode("Tu inscripción a ") . $evento['Nombre'] . " (" . $organiza . ")" . " fue reprobada. Comunicate con tu Representante Distrital.");
							}
						}
					}	
				}
	...
	function enviarCorreo($direccion, $asunto, $cuerpo){
		if ($direccion != ""){
			$mail							= new PHPMailer();
			$mail->CharSet = 'UTF-8';
			$mail->IsSMTP();

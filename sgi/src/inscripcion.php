...
//&& $esOrganizador != "SI"
					...
					} else if ($socio['Rueda'] == "Rotary" && $socio['NombreDistrito'] != "Otro"){
...
//&& $esOrganizador != "SI"
					...
					} else if ($socio['NombreDistrito'] == "Otro"){
				...
				} else if ($datosEvento['FechaInicioInscripcion2'] < $hoy && $datosEvento['FechaFinInscripcion2'] > $hoy){
					//FASE II
					...
					if ($accion != "inscripcionCN"){ //No es cambio de nombre
						...
						} else if ($disponible <= 0 && $esOrganizador != "SI"){
							$espera = true;
						}
					}
				}
		...
		} else if ($accion == "inscripcionE" || $accion == "inscripcionC"){ //Acepta edición de inscripción o cambio de nombre
			$idEvento=$_POST['idEvento'];
			...
			if (!$sinLugar){
				...
				if ($aprobado['Aprobado'] == 1){
					...
					} else if ($aprobado['Calidad'] != "Instructor" && $calidadActual['Nombre'] == "Instructor"){
						$conexion->Ejecuto("update historialevento set VecesInstructor=VecesInstructor+1 where idSocio=" . $idSocio . " and idTipoEvento=" . $tipoEvento['idTipoEvento']);
					}
					...
					while (!$entra){
						...
						if ($ingreso['Rueda'] == "Rotaract" && $ingreso['Distrito'] != "Otro"){ //Es Rotaractiano de AIRAUP el que sube
							...
							if ($cupoRotario > 0 && $cantidadR['Cantidad'] < $cupoRotario){ //Existe límite y queda lugar Rotarios
								$entra = true;
							} else if ($cupoRotario == 0){ //No existe límite
								$entra = true;
							}
							...
							if ($cupoExtranjeros > 0 && $cantidadE['Cantidad'] < $cupoExtranjeros){ //Existe límite y queda lugar extranjeros
								$entra = true;
							} else if ($cupoExtranjeros == 0){ //No existe límite
								$entra = true;
							}
						}
					}

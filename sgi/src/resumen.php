<?php
	...
	$tpl=new TemplatePower("resumen.html");
 	$tpl->prepare();
	...
	if ($idSocio == ""){
		...
		if ($admin['Admin'] == 1 || $representante || $adminEventos['Cantidad'] > 0 || $presidenteA['idSocio'] == $idSocio){
			...
			if ($hoy >= $eraup['FechaInicioInscripcion'] && $hoy <= $eraup['FechaFinInscripcion']){
				$tpl->assign("fase", "I");
			} else if ($hoy > $eraup['FechaFinInscripcion'] && $hoy < $eraup['FechaInicioInscripcion2']){
				$tpl->assign("fase", "stand by entre fases");
			} else if ($hoy >= $eraup['FechaInicioInscripcion2'] && $hoy <= $eraup['FechaFinInscripcion2']){
				$tpl->assign("fase", "II");
			} else if ($hoy > $eraup['FechaFinInscripcion2']){
				$tpl->assign("fase", "inscripción cerrada");
			} else {

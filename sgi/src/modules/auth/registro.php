...
if ($accion == "e") { //Editar datos personales
    if ($idSocio != "") {
        ...
        } elseif ($datosMedicos['GrupoSangre'] == "B") {
            $tpl->assign("seleccionadoB", "selected='selected'");
        } elseif ($datosMedicos['GrupoSangre'] == "O") {
            $tpl->assign("seleccionadoO", "selected='selected'");
        } elseif ($datosMedicos['GrupoSangre'] == "AB") {
    ...
    if ($idSocio == "") { //Es nuevo, hay datos que se piden solo en un registro nuevo
        ...
        if ($preregistro['idPreRegistro'] != "") {
            ...
            if ($idSocio != "") {
                ...
                //Inserto cantidad de asambleas y conferencias de AIRAUP a las que asisti�
            ...
            $tpl->newBlock("mensaje");
            $tpl->assign("mensaje", utf8_encode("No se pudo procesar tu registro dado que la transacci�n de preregistro no es v�lida. Por favor contactate con un administrador del sistema."));
        }
    } else {
        if ($editarCargos == "editarCargos") {
            $club=$_POST['club'];
            ...
            //Elimino registros de cargos y asistencias a mesa previo a insertar la informaci�n recibida
...
$conexion3->Libero(); //Se cierra la conexi�n a la base

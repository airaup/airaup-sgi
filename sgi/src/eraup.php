<?php
	...
	function calcularCuposDistrito($conexion, $conexion2, $idEvento, $idOrganizador){
		...
		//Asistencia global a los últimos 3 ERAUP
		...
		while ($distrito=$conexion->Siguiente()){
			...
			//Cantidad de Rotaractianos registrados del distrito en cuestión
	...
	function movimientosERAUP($conexion, $conexion2, $idEvento, $cantidad, $idPeriodoActual){
		...
		while ($x < $cantidad){ //Recorro todos los que están en lista de espera dentro del límite
			$entra = false;
			...
			if ($datosSocio['Rueda'] == "Rotaract" && $datosSocio['Distrito'] != "Otro"){ //Rotaractiano de AIRAUP
				$entra = true;
			} else if ($datosSocio['Rueda'] == "Rotary" && $datosSocio['Distrito'] != "Otro"){ //Rotario de AIRAUP
			...
			} else if ($datosSocio['Distrito'] == "Otro"){ //Extranjero

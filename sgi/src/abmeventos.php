			...
			if ($accion == "n"){
				...
				if ($idEvento == ""){ //Si no llega el id es un evento nuevo
					...
					foreach($_POST['admins'] as $check) {
						...
						enviarCorreo($correoA['Email'], utf8_encode("Potestad de administración"), utf8_encode("Fuiste designado como admnistrador del evento " . $nombre . " - " . $distritosOrg . "."));
					}
				} else { //De lo contrario se actualiza
					//Obtengo información actual del evento para comparar
					...
					for ($i=1;$i<=5;$i++){
						...
						//Actualizo o inserto la opción en la base
					...
					if ($representante){
						...
						foreach($_POST['admins'] as $check) {
							...
							if ($enviar){
								...
								enviarCorreo($correoA['Email'], utf8_encode("Potestad de administración"), utf8_encode("Fuiste designado como administrador del evento " . $nombre . " - " . $distritosOrg . "."));
							}
						}
						...
						for ($x=0;$x<$contar;$x++){
							if ($arrayAdmin[$x]['Existe'] == 0){
								...
								enviarCorreo($correoA['Email'], utf8_encode("Potestad de administración"), utf8_encode("Se te quitó la potestad de administración sobre el evento " . $nombre . " - (" . $distritosOrg . ")."));
							}
						}
					}
					...
					} else if (($cupo > $datosAnteriores['CupoMaximo']) && ($datosAnteriores['CupoMaximo'] < $inscriptosTotal['Cantidad'])){
	...
	function movimientoDeCupos($conexion, $conexion2, $idEvento, $cantBorrar, $idPeriodoActual, $tipo){
		...
		while ($espera=$conexion->Siguiente()){
			...
			if ($tipo == "positivo"){
				...
				if ($aprobado == 3){
					$mensajeI = utf8_encode("Los cupos del evento ") . $espera['Evento'] . utf8_encode(" se redujeron y quedaste en lista de espera. Tu inscripción sigue pendiente de aprobación.");
					$mensajeR = utf8_encode("Los cupos del evento " ). $espera['Evento'] . " se redujeron y " . $espera['Nombres'] . " " . $espera['Apellidos'] . utf8_encode(" quedó en lista de espera. Aún debes aprobar su inscripción.");
				} else if ($aprobado == 4){
					$mensajeI = utf8_encode("Los cupos del evento ") . $espera['Evento'] . utf8_encode(" se redujeron y quedaste en lista de espera. Tu inscripción fue aprobada previamente, en cuanto se liberen cupos te notificaremos.");
					$mensajeR = utf8_encode("Los cupos del evento ") . $espera['Evento'] . " se redujeron y " . $espera['Nombres'] . " " . $espera['Apellidos'] . utf8_encode(" quedó en lista de espera. Su inscripción fue aprobada previamente, en cuanto se liberen cupos te notificaremos.");
				}
			}
			...
			if ($tipo == "positivo"){
				...
				$asunto = utf8_encode("Tu inscripción pasó a estar en lista de espera");
			}
